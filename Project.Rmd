...
-609.40, -610.43 and -610.05
\pagebreak

# Models comparison and validation

Now that we have a selection of models, we need to check if they are valid and correctly fitted. To do so, we will first perform an univariate and two-sided significance test (based on normal approximations) of the coefficients. Then, we will perform an analysis of the residuals by a Portmanteau test and finally an evaluation of the predictive capacity "on sample". 
...
# Test the coefficients of each model
...
Thanks to the indicators in parentheses, it is easy to see which coefficients are significant and to what degree. First, we notice that for the model with the lowest AIC (1), we cannot reject the hypothesis that the coefficient sar1 ($\Phi_1$) is statistically different from zero, even with an $\alpha = 0.10$. The same is observed for the third model with the coefficient ar2 ($\phi_2$).

Those results lead us to choose the second model. This one being the most parsimonious, it only loses 0.064 points of AIC and in view of the significance test seems to be the most relevant. The latter can mathematically be written as :
...
This time we will analyze the standardized residuals and test if they are different from white noise.
