...
Considering our data to be an additive model $Y_t = T_t + S_t + \epsilon_t$, we can have a first insight by decomposing the series into trend ($T$), seasonal ($S$) and random ($\epsilon$) pieces. A plot containing the undecomposed data is available in the [appendix](#F1).
...
abline(reg = lm(gas.1 ~ tail(gas.t, -1)), col = "red", lty = 1)
abline(h=0, col = rgb(0, 0, 1, .7), lty = 2)
legend("topright", legend = c("Linear trend", "Horizontal line"), col = c("red","blue"), cex = .8, lty = c(1,2))

```

We observe in the figure above that once the data are differentiated, the linear trend is much less pronounced but we still have a visible periodicity. This particularity seems to be corrected on the second graph below as we now have a time series that appears to be random. Although we notice by plotting a linear regression (in red) that the trend is not quite equal to zero, the slope is flat enough for us to consider the time series to be (weakly) stationary.
...
In consideration of the aforementioned elements, we decided to proceed with an SARIMA model. Before continuing, we should yet provide a small definition. Basically, an SARIMA is an Autoregressive Integrated Moving Average that supports the direct modeling of a Seasonal component (S). Meaning we have 4 more elements ($P$, $D$, $Q$) and $s$, where $P$ is the seasonal AR order, $D$ the seasonal difference order, $Q$ the seasonal MA order and $s$ the seasonal period. 
...
Now it's time to figure out which model would be the best fit to the data. To do this, we will iterate over all possible SARIMA models with parameters value up to 2. Nevertheless, following the visual analysis of the previous section, we expect to get models going only up to 1.

In order to compare the effectiveness of the different models, we will primarily use the Akaike information criterion (AIC). However, since the AIC tends to overestimate the number of parameters needed, we will also use the Bayesian information criterion (BIC), which penalizes a bit more the number of parameters, as a secondary information.
...
model.1 = sarima.model.selection(gas, max.pq = c(2,2), max.PQ = c(1,1), d = 1, D = 1, top=3, return.best = T)
...
As a last test, we will check the autocorrelation of the residuals of the model via a portmanteau test, more specifically Ljung-box. We test the null hypothesis that the residuals are not different from white noise, up to lag $K = \sqrt{T}$. 
...
```
\vspace{-1em}
As the p-values are all well above the threshold, we cannot reject the null hypothesis. This is convenient for us because it means that there is no correlation in the residuals. We also tested the residuals squared and the [result](#F3) is similar. A more complete diagnostic which includes the ACF of the residuals is available in the [appendix](#F2).
...
hw = plot.n.ahead.predictions(gas, model.2, n=gas.freq+4, before=8, holtwinters = T)

```

```{r plot_predictions_comparison, appendix=TRUE}
(function(){
plot(md$pred, lwd=2, ylim=c(min(md$pred-1000), max(md$pred+800)), main="Predictions comparison", col=1, ylab="MMcf")
...
\begin{mdframed}[style=thicc, frametitle=Note, frametitlebackgroundcolor=black!30]
  For reproducibility purposes, the complete project containing the source code, the full sized figures and the results is available on \href{https://github.com/lamylio/LSTAT2170-Project}{github.com/lamylio}.
\end{mdframed}
...
```{r, appendix=TRUE, fig.height=5.5}
...
plot.ljungbox(resid(model.2)**2, floor(sqrt(length(gas))), title="Ljung-Box of the squared residuals")

```


### QQ-Plot of the residuals of model 2{#F4}
```{r, appendix=TRUE}
...
### Predictions not zoomed

```{r, appendix=TRUE, out.width="49%", fig.width=5.5}
# Predictions with model 2 and Holt-winters, zoomed
