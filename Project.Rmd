...
model.aic = sarima_model_selection(gas, max.pq = c(2,2), max.PQ = c(1,1), d = 1, D = 1, top=3)

# Second best model (best bic)
model.bic = arima(gas, order = c(1, 1, 1), seasonal = list(order = c(0, 1, 1), period = gas.freq))

```
\pagebreak

So we will continue with the two best models, namely: 

S-ARIMA(1,1,1)$\times$(1,1,1)$_{12}$
$$
(1-\phi_1B)(1-\Phi_1B^{12})(1-B)^1(1-B^{12})^1\ Y_t = (1+\theta_1B)(1+\Theta_1B^{12})\ \epsilon_t
$$

with coefficients $\phi_1 = 0.446$, $\Phi_1 = -0.190$, $\theta_1 = -0.835$, and $\Theta_1 = -0.869$
