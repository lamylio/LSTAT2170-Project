...
Exponential smoothing methods produce forecasts where recent observations have a more exponentially greater weight than past ones. In other words, the more recent the observation, the higher the associated weight. Moreover, they can be used in a wide range of time series without any particular conditions. Therefore, it would be interesting to compare one of these methods before concluding with a SARIMA model. However, as the data contains both trend and seasonal components, the Holt (1957) and Winters (1960) method will be used. The component form, for an additive model is, with $p$ the seasonality:
...
\end{split}
$$
and correspond to respectively the level, the trend and the seasonal component with their smoothing parameters $\alpha$, $\beta$ and $\gamma$. Those parameters are determined by minimizing the squared prediction error. The full output is available in [the appendix](#O1). With the gas deliveries time series, the latter are equal to:
...
md = plot.n.ahead.predictions(gas, model.2, n=gas.freq+4, before=8, xlab="Year", ylab="MMcf")
hw = plot.n.ahead.predictions(gas, model.2, n=gas.freq+4, before=8, holtwinters = T, xlab="Year", ylab="MMcf")
...
```{r, appendix=TRUE, out.width="49%", fig.width=5.5, fig.show="hold", results='hide'}
# Predictions with model 2 and Holt-winters, zoomed
